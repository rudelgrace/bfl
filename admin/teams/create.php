...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if (empty($errors)) {
        ...
        $dupCheck = getDB()->prepare('SELECT id FROM teams WHERE league_id = ? AND LOWER(name) = LOWER(?) AND id != ?');
        $dupCheck->execute([$leagueId, $name, $isEdit ? $teamId : 0]);
        if ($dupCheck->fetch()) {
            $errors[] = 'A team named "' . htmlspecialchars($name) . '" already exists in this league.';
        }
    }
